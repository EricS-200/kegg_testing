    ...
    if mappings_all_path.is_file():
        ...
        if mapping_file_extension == ".json":
            ...
            with ProcessPoolExecutor(max_workers=16) as exe:
                futures = [exe.submit(process_pair, rxn_pair, atom_mapping_dict[rxn_pair], reactions_all_path, cpds_all_path, kcf_path, rclass_path) for rxn_pair in atom_mapping_dict.keys()]

                for future in tqdm(as_completed(futures), total=len(futures), desc="Processing"):
                    res = future.result()
                    if res != None:
                        rxn_pairs_info_dict_all[res[1]] = res[0]
    else:
...
def process_pair(rxn_pair: str, pair_atom_map, reactions_all_path: str, cpds_all_path: str, kcf_path: str, rclass_path: str) -> tuple[dict, str] | None:
